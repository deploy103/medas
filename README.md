...
- 저장소 사용량과 남은 용량 표시
- 여러 파일 선택 후 한 번에 공유
- 공유 링크 생성 시 전체 다운로드용 ZIP 사전 생성
- 공유됨 탭에서 링크, 만료까지 남은 시간, 포함 파일 확인
- 공유 받은 사람용 파일 목록, 개별 다운로드, 모두 다운로드
...
- 공유 URL은 `/s/{token}`입니다.
- 전체 다운로드는 `/s/{token}/download`에서 ZIP으로 제공됩니다.
- 개별 파일 다운로드는 `/s/{token}/files/{share_file_id}/download`에서 제공합니다.
- 공유 토큰은 허용 문자와 길이를 검증합니다.
- 공개 공유 페이지는 파일명, 크기, 업로드 날짜, 만료일만 보여줍니다.
- ZIP 내부 파일명은 경로 구분자와 제어 문자를 제거해 경로 조작을 막습니다.
